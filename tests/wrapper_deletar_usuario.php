<?php
/**
 * Wrapper for testing deletar_usuario.php without side effects.
...
class MockDBStreamDeletarUsuario {
    private $position;
    private $content;
    ...
    public function stream_open($path, $mode, $options, &$opened_path) {
        ...
        if (strpos($realPath, 'config/db_conexao.php') !== false) {
            $this->content = "<?php
                ...
                class MockStatement {
                    ...
                    public function __construct(\$query, &\$conn) {
                        ...
                        // Default mock logic depending on query
                        if (strpos(\$query, 'SELECT username FROM usuarios') !== false) {
                            if (isset(\$conn->state['user_exists']) && \$conn->state['user_exists']) {
                                \$this->result = new MockMySQLiResult([['username' => 'testuser']]);
                            } else {
                ...
                class MockMySQLi {
                    ...
                    public function prepare(\$query) {
                        if (isset(\$this->state['db_prepare_error_queries'])) {
                            foreach (\$this->state['db_prepare_error_queries'] as \$err_q) {
                                if (strpos(\$query, \$err_q) !== false) {
                                    \$this->error = \"Prepare failed for: \$query\";
                                    return false;
                                }
                            }
                        }
            ...
            if (file_exists($realPath)) {
                ...
                stream_wrapper_register("file", "MockDBStreamDeletarUsuario");
                return false;
            }
            stream_wrapper_unregister("file");
            stream_wrapper_register("file", "MockDBStreamDeletarUsuario");
...
ob_start();
try {
    include __DIR__ . '/../deletar_usuario.php';
} catch (Throwable $e) {
