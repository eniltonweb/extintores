...
require_once __DIR__ . '/../limpar_historico_inspecao.php';

class LimparHistoricoInspecaoTest extends MiniTestCase {
    ...
    public function testRedirectsWhenNoSession() {
        ...
        $result = limpar_historico_inspecao_logic($conn, $session, $post, $server);
    ...
    public function testRedirectsWhenNotPostMethod() {
        ...
        $result = limpar_historico_inspecao_logic($conn, $session, $post, $server);
        $this->assertEquals('Location: historico_inspecao.php', $result);
    }
    ...
    public function testClearsHistorySuccessfully() {
        $conn = new class extends MockConnection {
            ...
            public function query($query) {
                $this->queries[] = $query;
                if (strpos($query, 'UPDATE bd_extintores SET') !== false) {
        ...
        $this->assertTrue(strpos($result, 'message=Hist%C3%B3rico+de+inspe%C3%A7%C3%A3o+limpo+com+sucesso') !== false);
    ...
    public function testHandlesDatabaseFailure() {
        ...
        $this->assertTrue(strpos($result, 'message=Erro+ao+limpar+o+hist%C3%B3rico+de+inspe%C3%A7%C3%A3o.') !== false);
    }
}
?>
