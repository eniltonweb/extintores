...
function auditoria($acao, $codigo_extintor, $user_id, $user_level, $detalhes = '') {
    ...
    if (!is_null($codigo_extintor)) {
        if (array_key_exists($codigo_extintor, $extintor_cache)) {
            ...
            if ($stmt_extintor !== false) {
                ...
                error_log("Erro ao preparar statement para buscar id do extintor: " . ($conn->error ?? 'Unknown error'));
            }
        }
    }
    ...
    if ($stmt !== false) {
        ...
        error_log("Erro ao preparar statement para auditoria: " . ($conn->error ?? 'Unknown error'));
    }
}
...
if (!function_exists('registrar_auditoria')) {
    function registrar_auditoria($conn, $user_id, $action, $details) {
        ...
        if ($stmt !== false) {
            ...
            error_log("Erro ao preparar statement para registrar_auditoria: " . ($conn->error ?? 'Unknown error'));
        }
    }
}
?>
